...
さらに磁場の統計誤差を含んだ$g$ 因子の統計誤差については，誤差伝播の式，式\eqref{eq:PSgosa} を用いて計算した．
\begin{equation}
...
\label{eq:PSgosa}
\end{equation}%
$\delta B = 0.01~\mathrm{G}$となったので，$\delta g$ は表~\ref{tab:PSgStatErr} のようになった．
\begin{table}
	\centering
...
\begin{figure}[h]
	...
	\caption{$\sigma_{x}, \;\sigma_{y}$ を1~mm 刻みで動かしたときの磁場の加重平均の値．中心付近の点はいただいていたビームプロファイルでの値を示している．最大値をとったのは$\sigma_{x} = 3.9, \;\sigma_{y} = 1.0$ のとき，最小値をとったのは$\sigma_{x} = 3.9, \;\sigma_{y} = 2.9$ のときであった．}
