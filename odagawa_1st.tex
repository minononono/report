...
磁場に誤差は二種類あり，一つは各測定点の統計誤差を加重平均にて伝播させたもの，もう一つはビームプロファイルの不定性に由来するものである．
...
\end{equation}%
まず前者の系統誤差について記す．
磁場の統計誤差は予備実験から$\delta B = 0.01~\mathrm{G}$である．これを式~\eqref{eq:PSgosa}で伝播させた結果，各層における$g$~因子の誤差は$0.001$より小さくなった．
この値は最小有効桁より小さいので，後者の系統誤差のみを考えることにした．
